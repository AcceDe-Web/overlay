...
(function(window, document, exportName) {
  ...
  var Modal = {
    ...
    bind: function( modal ){
      ...
      // listen for the escape key keypress
      else{
    ...
    createDom: function( options ){
      ...
      // look for a label inside the modal
      else if( options.role ){
        var title = el.querySelector( '[data-label]' ),
            titleId;

        if( title ){
          titleId = title.id || 'modal'+( this.modalStack.length + 1 );
    ...
    validateModal: function( modal ){
      ...
      if( !modalTitle || !modal.contains( modalTitle ) ){
        throw new Error( 'No title is present in the modal.' );
      }
    }
  };
