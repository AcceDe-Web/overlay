...
const cancelListener = event => {
  if( !modalStack.length || event.keyCode !== 27 ){
    return;
  }

  const topModal = modalStack[ 0 ].modal;
...
class Modal {
  ...
  _handleClose(){
    ...
    if( this.options.modal && Modal.stack.length === 1 ){
      // enable focusable elements
      this._enableDocument();
    }

    // update the modal stack
    Modal.stack.splice( Modal.stack.indexOf( this.el ), 1 );
   ...
   * @param {string} returnValue the value the modal will return
   */
  close( returnValue ){
  ...
  /**
   * Init modal
   * @param {HTMLElement} el Complete modal DOM element
   * @returns {Object} Modal instance
   */
  _init( el ){
    // update the modal index
    Modal.modalIndex++;
   ...
   */
  show(){
    // add the new modal at the top of the stack
    Modal.stack.unshift( this.el );

    // set the default returnValue of the modal
    this.returnValue = '';
    ...
    if( Modal.stack.length === 1 && this.options.modal && !this.documentDisabled ){
  ...
  _validateModal(){
    ...
    if( !label || ( label && label.trim().length < 0 )) {
      ...
      if( !modalTitle || !this.el.contains( modalTitle )) {
        throw new Error( 'No title is present in the modal. Use the "data-label" attribute on the visible title or pass the label using the "label" key when creating the modal.' );
      }
    }
...
// listen to the escape key to close the top-most modal
