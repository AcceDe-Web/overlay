...
class Modal {
  ...
  set returnValue( value ){
    ...
    // store the returnValue on the HTMLElement
    this.el.returnValue = this._returnValue;
  }
   ...
   */
  _handleClose( event ){
    // don't close on cancel if role is 'alertdialog' and event comes from a keyboard event
    if( this.options.role === 'alertdialog' && event.constructor.name === 'KeyboardEvent' && this.returnValue === 'cancel' ){
      return;
    }
   ...
   * @param {KeyboardEvent} event `keydown` DOM event
   */
  _handleKeydown( event ){
    // close the modal on escape press if its not a dialog tag
    if( event.keyCode === 27 && this.options.closeOnCancel ){
